# coding=utf-8
# Copyright 2018 The Google AI Language Team Authors and The HuggingFace Inc. team and Jianiu.lj.
...
from transformers import (WEIGHTS_NAME, AdamW, get_linear_schedule_with_warmup,
                                  BertConfig, BertForMaskedLM, BertForDialogMaskedLM, BertTokenizer,
...
MODEL_CLASSES = {
    ...
    'bert_dialog': (BertConfig, BertForDialogMaskedLM, BertTokenizer),
...
def mask_tokens(inputs, tokenizer, args):
    ...
    probability_matrix.masked_fill_(torch.tensor(mask_ids==0, dtype=torch.uint8), value=0.0)
    masked_indices = torch.bernoulli(probability_matrix).type(torch.uint8)
    labels[~masked_indices] = -1  # We only compute loss on masked tokens

    # 80% of the time, we replace masked input tokens with tokenizer.mask_token ([MASK])
    indices_replaced = torch.bernoulli(torch.full(labels.shape, 0.8)).type(torch.uint8) & masked_indices
    inputs[indices_replaced] = tokenizer.convert_tokens_to_ids(tokenizer.mask_token)

    # 10% of the time, we replace masked input tokens with random word
    indices_random = torch.bernoulli(torch.full(labels.shape, 0.5)).type(torch.uint8) & masked_indices & ~indices_replaced
...
def train(args, train_dataset, model, tokenizer):
    ...
    tr_right, logging_right = 0, 0
    tr_wrong, logging_wrong = 0, 0
    model.zero_grad()
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.model_type == 'bert_dialog':
                ...
                if args.n_gpu > 1:
                    ...
                    loss = achieved_loss * args.achieved_loss_weight + lm_loss
                right_num = (achieved_logits.max(-1)[1] == achieved_label).sum()
                ignore_num = (achieved_label == -1).sum()
                wrong_num = ((achieved_logits.max(-1)[1] != achieved_label) & (achieved_label != -1)).sum()
                assert right_num + ignore_num + wrong_num == len(achieved_label)
                tr_right += right_num.item()
                tr_wrong += wrong_num.item()
            else:
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.model_type == 'bert_dialog':
                        acc = (tr_right - logging_right)/(tr_right - logging_right + tr_wrong - logging_wrong) if (tr_right-logging_right > 0) else 0
                        tb_writer.add_scalar('acc', acc, global_step)
                        logging_right = tr_right
                        logging_wrong = tr_wrong
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    eval_loss = 0.0
    nb_eval_steps = 0
    achieved_right_num, achieved_ignore_num, achieved_wrong_num = 0, 0, 0
    model.eval()
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            if args.model_type == 'bert_dialog':
                ...
                wrong_num = ((achieved_logits.max(-1)[1] != achieved_label) & (achieved_label != -1)).sum()
                assert right_num + ignore_num + wrong_num == len(achieved_label)
                achieved_right_num += right_num.item()
                achieved_ignore_num += ignore_num.item()
                achieved_wrong_num += wrong_num.item()
            else:
                outputs = model(inputs, masked_lm_labels=labels) if args.mlm else model(inputs, labels=labels)
    ...
    result = {
        ...
        "achieved_right": achieved_right_num,
        "achieved_wrong_num": achieved_wrong_num,
        "achieved_ignore_num": achieved_ignore_num,
        "achieved_acc:": achieved_right_num/(achieved_right_num + achieved_wrong_num) if achieved_right_num > 0 else 0
    }
...
def main():
    ...
    parser.add_argument("--model_type", default="bert_dialog", type=str,
    ...
    parser.add_argument("--achieved_loss_weight", type=float, default=1.0,
                        help="Weight for achieved label loss")
